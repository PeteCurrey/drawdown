...
export default async function IntelligenceHub() {
  // ── Auth + tier check (Bug C: independent of dashboard card state) ────────
  // A free user who navigates directly to /dashboard/intelligence must see the
  // same locked state as if they'd tried to click the card on the dashboard.
  ...
  // Intelligence Hub requires Edge (weight 2). Data fetches below are intentionally
  // skipped for locked users to avoid wasting API calls.
  if (userWeight < 2) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-8 animate-in fade-in duration-700">
        <div className="p-10 bg-background-surface/40 backdrop-blur-md border border-border-slate/50 flex flex-col items-center text-center space-y-6 max-w-md w-full">
          ...
          <div className="space-y-2">
            <p className="text-xs font-bold uppercase tracking-widest text-text-primary">Edge Access Required</p>
            <p className="text-xs text-text-secondary leading-relaxed">
              The Intelligence Hub includes insider transaction monitoring, political capital
              flow tracking, and AI signal synthesis. Available on Edge and Floor plans.
              Your current plan is{" "}
              <span className="font-bold text-text-primary uppercase">{tier ?? "Free"}</span>.
            </p>
          </div>
        ...
        <p className="text-[9px] font-mono text-text-tertiary/50 uppercase tracking-widest">
          Intelligence Hub · Edge+ · Insider &amp; AI Signal Data
        </p>
      </div>
    );
  }

...
export function LiveDashboardPreview() {
  ...
  return (
    <section className="py-12 md:py-20 bg-background-primary relative z-10 -mt-20">
      <div className="container mx-auto px-6">
        <div className={cn(
          "bg-background-surface border border-border-slate rounded-xl overflow-hidden shadow-2xl transition-all duration-700",
          ...
          <div className="grid grid-cols-1 lg:grid-cols-10 divide-y lg:divide-y-0 lg:divide-x divide-border-slate/50">
            ...
            <div className="lg:col-span-3 p-8 space-y-8 bg-background-primary/30">
                ...
                {markets.map((item) => (
                  <div key={item.symbol} className="flex items-center justify-between group cursor-pointer hover:bg-white/5 p-2 -m-2 transition- premium active:scale-95">
                    <div className="flex flex-col">
                      <span className="text-sm md:text-xs font-bold font-mono text-text-primary uppercase tracking-tighter">{item.symbol}</span>
              ...
              <Link href="/markets" className="flex items-center justify-center gap-2 py-4 md:py-4 bg-background-elevated border border-border-slate text-sm md:text-[9px] font-bold uppercase tracking-widest hover:border-accent hover:text-accent transition-all">
            ...
            <div className="lg:col-span-3 p-10 space-y-10 bg-background-primary/30">
                ...
                <div className="p-8 bg-background-elevated border-l-4 border-l-loss border border-border-slate space-y-6 relative overflow-hidden group">
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-6 mt-6 border-t border-border-slate/50 relative z-10">
                    <div className="flex flex-col">
                      ...
                      <span className="text-sm md:text-xs font-mono font-bold text-text-primary">{nextEvent.forecast || "—"}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[8px] font-mono text-text-tertiary uppercase font-bold tracking-tighter">Previous</span>
                      <span className="text-sm md:text-xs font-mono text-text-secondary">{nextEvent.previous || "—"}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-6">
                ...
                {upcomingEvents.map((event, i) => (
                  <div key={i} className="flex items-center justify-between group cursor-default">
                    <div className="flex flex-col">
                      <span className="text-sm md:text-xs font-bold uppercase text-text-secondary group-hover:text-text-primary transition-colors">{event.event}</span>
              ...
              <Link href="/markets?tab=calendar" className="flex items-center justify-center gap-2 py-4 md:py-4 bg-background-elevated border border-border-slate text-sm md:text-[9px] font-bold uppercase tracking-widest hover:border-accent hover:text-accent transition-all mt-auto">

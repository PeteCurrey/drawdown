...
export default function BacktesterPage() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-24">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-border-slate/50 pb-8">
        <div>
          <span className="text-accent font-mono text-[10px] uppercase tracking-widest block mb-2">// STRATEGIC VALIDATION</span>
          <h1 className="text-3xl md:text-4xl font-display font-black uppercase text-text-primary">Backtester.</h1>
      ...
      <div className="max-w-6xl">
        {step === 'define' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="bg-background-surface border border-border-slate/50 p-8 md:p-12 space-y-8 rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.04)]">
              <div className="flex items-center gap-3 text-accent mb-2">
                <Terminal className="w-5 h-5" />
                <span className="text-[10px] font-mono uppercase font-bold tracking-widest">Strategy Definition (Voice to Code)</span>
              </div>
              <textarea 
                value={strategy}
                onChange={(e) => setStrategy(e.target.value)}
                placeholder="E.g. Buy when the 20-day EMA crosses above the 50-day EMA and RSI is below 40. Close when RSI hits 70..."
                className="w-full h-48 bg-background-primary border border-border-slate/80 p-6 text-sm font-sans text-text-primary focus:border-accent outline-none resize-none placeholder:text-text-tertiary/50 leading-relaxed rounded-lg"
              />
              <div className="flex items-center gap-6">
                <button 
                  onClick={() => setStep('params')}
                  disabled={!strategy}
                  className="px-8 py-4 bg-[#0A0A0A] hover:bg-neutral-800 text-white text-[10px] font-bold uppercase tracking-widest transition-colors disabled:opacity-50 rounded-lg"
                >
                  Confirm Logic
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="p-8 bg-accent/5 border border-accent/20 space-y-4 rounded-xl">
              ...
              <div className="p-8 border border-border-slate/50 space-y-4 flex items-center gap-4 text-left rounded-xl bg-background-surface/40">
        ...
        {step === 'params' && (
          <div className="bg-background-surface border border-border-slate/50 p-8 md:p-12 animate-in fade-in slide-in-from-right-4 duration-700 rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.04)]">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
                <div className="space-y-8">
                   <div className="space-y-2">
                      <label className="text-[10px] font-mono uppercase tracking-widest text-text-tertiary block">Select Instrument</label>
                      <select className="w-full bg-background-primary border border-border-slate/80 px-4 py-3 text-xs font-mono uppercase outline-none focus:border-accent text-text-primary rounded-lg">
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-mono uppercase tracking-widest text-text-tertiary block">Timeframe</label>
                      <div className="grid grid-cols-4 gap-2">
                         {['15M', '1H', '4H', '1D'].map(tf => (
                            <button key={tf} className="py-2 border border-border-slate text-[10px] font-bold text-text-secondary hover:border-accent hover:text-accent transition-colors rounded-lg">{tf}</button>
                         ))}
                      </div>
                   </div>
                </div>
                <div className="space-y-8">
                   <div className="space-y-2">
                      <label className="text-[10px] font-mono uppercase tracking-widest text-text-tertiary block">Starting Capital (£)</label>
                      <input type="number" defaultValue={10000} className="w-full bg-background-primary border border-border-slate/80 px-4 py-3 text-xs font-mono outline-none focus:border-accent text-text-primary rounded-lg" />
                   </div>
                   <div className="space-y-2">
                      <label className="text-[10px] font-mono uppercase tracking-widest text-text-tertiary block">Data Range</label>
                      <div className="grid grid-cols-2 gap-4">
                         <input type="date" defaultValue="2024-01-01" className="bg-background-primary border border-border-slate/80 px-4 py-3 text-xs font-mono outline-none text-text-primary rounded-lg" />
                         <input type="date" defaultValue="2026-04-13" className="bg-background-primary border border-border-slate/80 px-4 py-3 text-xs font-mono outline-none text-text-primary rounded-lg" />
                      </div>
                   </div>
                </div>
             </div>
             ...
             <div className="flex items-center gap-6">
                <button 
                  ...
                  className="flex items-center gap-3 px-8 py-4 bg-[#0A0A0A] hover:bg-neutral-800 text-white text-[10px] font-bold uppercase tracking-widest transition-all rounded-lg"
                >
          ...
          <div className="space-y-8 animate-in fade-in zoom-in-95 duration-700">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                { label: "Win Rate", value: `${results.winRate.toFixed(1)}%`, color: "text-text-primary" },
                ...
                <div key={i} className="p-8 bg-background-surface border border-border-slate/50 text-center rounded-xl hover:shadow-[0_8px_32px_rgba(0,0,0,0.06)] transition-all">
            ...
            {/* Equity Chart */}
            <div className="bg-background-surface border border-border-slate/50 rounded-xl overflow-hidden hover:shadow-[0_8px_32px_rgba(0,0,0,0.04)] transition-all">
            ...
            {/* AI Strategy Coach */}
            <div className="p-10 bg-background-elevated/40 border border-border-slate/50 relative overflow-hidden rounded-xl">
               ...
               <div className="relative z-10 space-y-6 text-left">
                  ...
                  <div className="flex flex-col md:flex-row gap-4 pt-4">
                     <button onClick={() => setStep('define')} className="px-6 py-3 border border-accent hover:bg-accent/5 text-accent text-[10px] font-bold uppercase tracking-widest transition-all rounded-lg">Refine Strategy</button>
                     <button className="px-6 py-3 border border-border-slate/80 text-text-tertiary text-[10px] font-bold uppercase tracking-widest hover:text-text-primary transition-all rounded-lg">Download Detailed Report (PDF)</button>
                  </div>
               </div>
            </div>
          </div>

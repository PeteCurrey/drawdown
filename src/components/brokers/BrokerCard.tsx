...
export function BrokerCard({ broker, index, region = "uk" }: BrokerCardProps) {
  ...
  return (
    <div 
      className="bg-background-surface border border-border-slate overflow-hidden group hover:border-accent/30 transition-premium animate-in fade-in slide-in-from-bottom-4 duration-700"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12">
        ...
        <div className="lg:col-span-3 p-10 flex flex-col items-center justify-center border-b lg:border-b-0 lg:border-r border-border-slate bg-background-primary/30">
          <div className="w-24 h-24 bg-background-elevated flex items-center justify-center border border-border-slate mb-6 shadow-2xl relative overflow-hidden group-hover:scale-105 transition-transform">
             <div className="text-2xl font-display font-black text-accent/20">{broker.name.charAt(0)}</div>
        ...
        <div className="lg:col-span-6 p-10 space-y-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            {(broker.fcaRegulated || broker.regulation?.includes("ASIC")) && (
              <div className="flex items-center gap-2 px-3 py-1.5 bg-profit/10 border border-profit/20 rounded-none">
                ...
                <span className="text-[9px] font-mono font-bold text-profit uppercase tracking-widest">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 py-8 border-y border-border-slate/50">
            ...
            <div>
              ...
              <p className="text-sm font-bold font-mono text-text-primary">{broker.spreads || "0.0 Pips"}</p>
            </div>
            <div>
              <p className="text-[9px] font-mono text-text-tertiary uppercase tracking-widest mb-1">Platforms</p>
              <p className="text-sm font-bold font-mono text-text-primary">{broker.platforms?.length || 0}</p>
            </div>
            <div>
              <p className="text-[9px] font-mono text-text-tertiary uppercase tracking-widest mb-1">Max Leverage</p>
              <p className="text-sm font-bold font-mono text-text-primary">{broker.maxLeverage || "1:30"}</p>
            </div>
          </div>
              ...
              <ul className="space-y-2">
                {(broker.features || broker.pros || []).slice(0, 3).map((feature: string, i: number) => (
                  <li key={i} className="text-xs text-text-secondary leading-relaxed flex gap-2">
                    <span className="text-profit">•</span> {feature}
                  </li>
                ))}
              </ul>
            </div>
            <div className="space-y-4">
              <p className="text-[10px] font-mono text-text-tertiary uppercase tracking-widest flex items-center gap-2 font-bold">

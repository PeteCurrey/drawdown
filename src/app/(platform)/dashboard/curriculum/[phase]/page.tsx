...
export default async function PhaseOverviewPage({ params }: { params: Promise<{ phase: string }> }) {
  ...
  // ── Fetch DB data defensively — table may not be migrated yet ──
  let moduleList: any[] = [];
  ...
  if (totalCompleted >= totalModules) firstIncomplete = 1;
  ...
  return (
    <div className="max-w-5xl space-y-8 animate-in fade-in duration-500 pb-24">
      ...
      <header className="border-b border-[#222] pb-8">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-bold font-syne text-white tracking-tight">
              {phaseConfig.name}
            </h1>
        ...
        <div className="mt-8 bg-[#111] border border-[#222] rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="w-full">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-bold text-white">
                {pct}% ({totalCompleted}/{totalModules})
              </span>
            </div>
          ...
          >
            {totalCompleted >= totalModules ? (
              ...
              <><Play className="w-4 h-4 fill-black" /> {totalCompleted > 0 ? "Continue Phase" : "Start Phase"}</>
            )}
          </Link>
        </div>
      </header>
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-bold font-mono uppercase tracking-widest text-text-secondary mb-6">
          Modules in this Phase
        </h2>

        {!dbReady || moduleList.length === 0 ? (
          // Fall back to the static modules_list from courses.ts
          <div className="grid grid-cols-1 gap-3">
            {phaseConfig.modules_list.map((title, idx) => {
              const modNum = idx + 1;
              return (
                <Link
                  key={modNum}
                  href={`/dashboard/curriculum/${phaseConfig.slug}/module-${modNum}`}
                  className="flex flex-col md:flex-row md:items-center justify-between p-5 rounded-xl border bg-[#1A1A1A] border-[#333] hover:border-accent transition-all duration-200 group"
                >
                  <div className="flex items-center gap-5">
                    <div className="flex items-center justify-center w-10 h-10 rounded-full shrink-0 bg-[#222] text-text-secondary">
                      <span className="font-mono font-bold text-xs">{modNum}</span>
                    </div>
                    <div>
                      <h3 className="text-base font-bold text-white">{title}</h3>
                      <p className="text-xs text-text-tertiary mt-1">Content coming soon</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start md:items-center gap-5">
                    ...
                    >
                      {isCompleted ? (
                        ...
                        <span className="font-mono font-bold text-xs">{mod.module_number}</span>
                      )}
                    </div>
                    <div>

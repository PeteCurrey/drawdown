    ...
    <main className="min-h-screen bg-white pt-32 pb-20 px-6">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-20">
          ...
          <p className="text-xl text-mkt-i2 max-w-2xl font-sans leading-relaxed">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-24">
          <div className="lg:col-span-2 space-y-12">
             <div className="relative group">
                <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-mkt-i4 group-focus-within:text-accent transition-colors" />
                <input 
                  type="text" 
                  placeholder="Search the dictionary..." 
                  className="w-full bg-white border border-mkt-bd px-16 py-6 text-lg focus:border-mkt-bd outline-none transition-premium font-sans uppercase tracking-wider"
                />
             </div>

             <div className="space-y-6">
                <h4 className="text-[10px] font-mono uppercase tracking-[0.3em] text-mkt-i4">Quick Navigation</h4>
                <nav className="flex flex-wrap gap-2 border-y border-mkt-bd/30 py-8">
                  {alphabet.map((letter) => (
                    <a 
                      key={letter} 
                      href={`#${letter}`}
                      className={cn(
                        ...
                        groupedTerms[letter] 
                          ? "text-mkt-ink border-mkt-bd hover:border-mkt-bds hover:text-accent" 
                          : "text-mkt-i4 border-transparent pointer-events-none opacity-30"
                      )}
                    >
                      {letter}
                    </a>
             ...
             <div className="space-y-4">
                {trending.map((term) => (
                   <Link 
                     ...
                     className="flex items-center justify-between group py-2 border-b border-mkt-bd/30 last:border-0"
                   >
                      <span className="text-sm font-bold uppercase group-hover:text-accent transition-colors">{term.term}</span>
                      <ArrowRight className="w-3 h-3 text-mkt-i4 group-hover:translate-x-1 transition-all" />
                   </Link>
                ))}
             </div>
          </div>
          ...
          {alphabet.filter(l => groupedTerms[l]).map((letter) => (
            <section key={letter} id={letter} className="scroll-mt-32">
              <div className="flex items-center space-x-8 mb-12">
                <h2 className="text-8xl font-sans font-bold text-mkt-grn select-none leading-none">{letter}</h2>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
                {groupedTerms[letter].map((term) => (
                  <Link 
                    key={term.slug} 
                    href={`/glossary/${term.slug}`}
                    className="group flex flex-col justify-between py-6 border-b border-mkt-bd/50 hover:border-mkt-bds transition-all"
                  >
                    <div>
                       <h3 className="text-xl font-sans font-bold uppercase mb-4 group-hover:text-accent transition-colors">{term.term}</h3>
                       <p className="text-sm text-mkt-i2 leading-relaxed line-clamp-2 font-sans">
                         {term.definition}
                       </p>
                    </div>
                    <div className="mt-6 flex items-center justify-between">
                       <span className="text-[10px] font-mono uppercase tracking-widest text-mkt-i4 group-hover:text-accent transition-colors">Definition</span>
                       <ArrowRight className="w-4 h-4 text-mkt-i4 group-hover:translate-x-2 transition-all" />
                    </div>
                  </Link>
                ))}
              </div>

...
export function ScannerTab() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Search & Asset Bar */}
      <div className="flex flex-col md:flex-row justify-between gap-6 border-b border-mkt-bd pb-8">
              ...
              <button
                ...
                className={cn(
                  ...
                  isActive 
                    ...
                    : "bg-white text-mkt-i4 border border-mkt-bd hover:border-mkt-bds/40"
                )}
              >
        ...
        <div className="relative w-full md:w-80">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-mkt-i4" />
          <input 
            type="text"
            placeholder="Search symbols..."
            className="w-full bg-white border border-mkt-bd py-3 pl-12 pr-4 text-[10px] font-mono uppercase tracking-widest outline-none focus:border-accent transition-colors"
            value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {loading ? (
          Array(8).fill(0).map((_, i) => (
            <div key={i} className="h-40 bg-white/20 border border-mkt-bd animate-pulse" />
          ))
        ) : filteredPrices.length > 0 ? (
          filteredPrices.map((item, i) => (
            <div key={i} className="group p-6 bg-white border border-mkt-bd hover:border-mkt-bds transition-all duration-500">
              <div className="flex justify-between items-start mb-6">
                <div>
                   <h4 className="text-xl font-mono font-bold group-hover:text-accent transition-colors">{item.symbol}</h4>
                   <span className="text-[9px] font-mono text-mkt-i4 uppercase tracking-widest">
                     Vol: {(item.volume / 1000).toFixed(1)}K
                   </span>
                </div>
                <div className={cn(
                  "px-2 py-1 text-[10px] font-mono font-bold border",
                  item.change >= 0 ? "text-mkt-grn border-profit/20 bg-profit/5" : "text-red-500 border-loss/20 bg-loss/5"
                )}>
          ...
          <div className="col-span-full py-20 text-center border border-dashed border-mkt-bd/50">
             <Percent className="w-10 h-10 text-mkt-i4 mx-auto mb-4 opacity-20" />
             <p className="text-[10px] font-mono uppercase text-mkt-i4 tracking-widest">No active technical results for this search // 0x00</p>
          </div>
        )}
      </div>

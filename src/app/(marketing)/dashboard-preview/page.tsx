...
export default function DashboardConceptPage() {
  return (
    <div className="flex flex-col bg-white min-h-screen pt-24">
      <div className="max-w-7xl mx-auto px-6 py-12 max-w-7xl">

        {/* Page Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 pb-12 border-b border-mkt-bd">
          <div>
            <p className="text-[10px] font-mono uppercase tracking-widest text-mkt-i4 mb-2">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {dashboardModules.map((mod, i) => (
            <div key={i} className="bg-white border border-mkt-bd p-6 hover:border-mkt-bds/20 transition-colors">
              <p className="text-[10px] font-mono uppercase tracking-widest text-mkt-i4 mb-3">{mod.label}</p>
              <div className={`text-4xl font-sans font-black mb-2 ${mod.color}`}>
                {mod.value}<span className="text-xl text-mkt-i2">{mod.unit}</span>
              </div>
              <p className="text-[10px] font-mono text-mkt-i4">{mod.note}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          {/* Equity Curve (Large) */}
          <div className="lg:col-span-2 bg-white border border-mkt-bd p-6 flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-sm font-bold uppercase tracking-widest text-mkt-ink">Equity Curve</h2>
              <div className="flex gap-2">
                {["1W","1M","3M"].map(t => (
                  <button key={t} className={`text-[10px] font-mono px-3 py-1 border transition-colors ${t === "1M" ? "border-mkt-bd text-accent" : "border-mkt-bd text-mkt-i4 hover:text-mkt-ink hover:border-text-primary"}`}>{t}</button>
                ))}
              </div>
            </div>
            {/* Placeholder curve */}
            <div className="flex-grow bg-white border border-mkt-bd/30 rounded-sm flex items-center justify-center min-h-[200px] relative overflow-hidden">
              ...
              <div className="relative z-10 text-center">
                ...
                <p className="text-[10px] font-mono text-mkt-i4 uppercase tracking-widest opacity-60">Live data synced from journal</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="bg-white border border-mkt-bd p-6">
              ...
              <p className="text-sm text-mkt-i2 leading-relaxed mb-4 font-medium">
            ...
            {/* Macro Events */}
            <div className="bg-white border border-mkt-bd p-6">
              <h3 className="text-sm font-bold uppercase tracking-widest text-mkt-ink mb-4">This Week That Matters</h3>
              <div className="space-y-3">
                {[
                  { time: "Thu 13:30", event: "US Core PCE m/m", impact: "HIGH" },
                  { time: "Fri 07:00", event: "UK GDP q/q", impact: "MED" }
                ].map((ev, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b border-mkt-bd/40 last:border-none">
                    <div>
                      <p className="text-xs font-mono text-mkt-i4">{ev.time}</p>
                      <p className="text-sm text-mkt-ink font-bold">{ev.event}</p>
                    </div>
        ...
        {/* Action Items */}
        <div className="bg-white border border-mkt-bd p-6">
          <h2 className="text-sm font-bold uppercase tracking-widest text-mkt-ink mb-6 flex items-center gap-2">
        ...
        <div className="mt-8 bg-white border border-premium/20 p-6 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row gap-6 items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-mkt-ink font-bold uppercase tracking-widest text-sm mb-1">AI Performance Attribution</h3>
                <p className="text-xs text-mkt-i2">Unlock your full breakdown of win rate by session, setup, and psychological state.</p>
              </div>
            </div>

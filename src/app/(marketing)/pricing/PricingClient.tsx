...
const tiers = [
  ...
  },
  {
    ...
    borderAccent: "rgba(200, 241, 53, 0.5)",
    savings: "720",
    features: [
...
export default function PricingPage({ floorCap = 15, activeFloorSubs = 0 }: { floorCap?: number, activeFloorSubs?: number }) {
  ...
  return (
    <div className="pt-28 pb-24 min-h-screen bg-[#04060A] text-slate-100 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center justify-center gap-4 mb-14">
          <span className={cn("text-sm font-sans font-medium transition-colors", billingCycle === "monthly" ? "text-white font-bold" : "text-slate-400")}>
            Monthly Billing
          </span>
          <button
            onClick={() => setBillingCycle(prev => prev === "monthly" ? "yearly" : "monthly")}
            className="w-14 h-7 bg-slate-900 border border-slate-700 rounded-full p-0.5 relative transition-colors cursor-pointer"
          >
            <div
              className="absolute top-0.5 left-0.5 w-6 h-6 bg-[#C8F135] rounded-full transition-transform duration-300 shadow-md"
              style={{ transform: billingCycle === "yearly" ? "translateX(28px)" : "translateX(0)" }}
            />
          </button>
          <span className={cn("text-sm font-sans font-medium transition-colors flex items-center gap-1.5", billingCycle === "yearly" ? "text-white font-bold" : "text-slate-400")}>
            Yearly Billing
            <span className="text-[10px] font-mono font-bold text-[#C8F135] bg-[#C8F135]/15 px-2 py-0.5 rounded border border-[#C8F135]/30">
              SAVE 20%
            </span>
          </span>
        </div>
          ...
          {tiers.map((tier) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "relative flex flex-col bg-[#070A12] border rounded-xl overflow-hidden transition-all duration-300",
                  tier.highlight ? "shadow-[0_0_35px_rgba(6,182,212,0.15)] border-[#06b6d4]/50" : "border-slate-800",
                )}
                style={{
                  borderColor: isHovered ? tier.borderAccent : tier.highlight ? tier.borderAccent : "rgba(30, 41, 59, 0.8)",
              ...
              >
                ...
                {tier.badge && (
                  <div className={cn(
                    ...
                    tier.name === "Edge" ? "bg-[#06b6d4] text-black" : "bg-[#C8F135] text-black"
                  )}>
                    {tier.badge}
                  </div>
                )}
                ...
                <div className="relative z-10 p-6 flex flex-col flex-1">
                  ...
                  <div className="mb-6 space-y-1">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xl font-black text-white font-sans tracking-tight">{tier.name}</h3>
                      {tier.name === "Floor" && (
                        <span className="text-[9px] font-mono font-bold text-[#C8F135] bg-[#C8F135]/15 border border-[#C8F135]/30 px-2 py-0.5 rounded">
                          ALL INCLUDED
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-slate-400 font-sans leading-relaxed min-h-[32px]">{tier.description}</p>
                  </div>
                  ...
                  <div className="mb-6 pb-6 border-b border-slate-800">
                    ...
                    {tier.name === "Floor" && (
                      <p className="text-[10px] font-mono text-[#C8F135] mt-1 font-bold">
                        ★ Includes £99/mo Investment Centre FREE
                      </p>
                    )}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      tier.highlight
                        ? "bg-[#06b6d4] text-black hover:bg-[#0891b2] shadow-[0_0_20px_rgba(6,182,212,0.3)]"
                        : tier.name === "Floor"
                        ? "bg-[#C8F135] text-black hover:bg-[#b3d82a] shadow-[0_0_20px_rgba(200,241,53,0.3)]"
                  ...
                  <div className="space-y-3 pt-2 flex-1">
                    ...
                    {tier.features.map((feature, i) => (
                      <div key={i} className="flex items-start gap-2 text-xs">
                        {feature.included ? (
                          <Check className={cn("w-4 h-4 shrink-0 mt-0.5", (feature as any).accent ? "text-[#C8F135]" : "text-emerald-400")} />
                        ) : (feature as any).tierNote ? (
                          <span className="w-4 h-4 shrink-0 mt-0.5 flex items-center justify-center text-[10px] text-slate-600 font-mono">◎</span>
                        ) : (
                          <X className="w-4 h-4 text-slate-600 shrink-0 mt-0.5" />
                        )}
                        <span className={cn("font-sans leading-relaxed flex-1", feature.included ? "text-slate-200" : "text-slate-500")}>
                          {feature.name}
                        </span>
                        {(feature as any).tierNote && (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-bold font-mono uppercase tracking-wide text-slate-400 bg-slate-900 border border-slate-800 shrink-0 ml-1">
                            {(feature as any).tierNote}
                          </span>
                        )}
                        {(feature as any).badge && !(feature as any).tierNote && (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-bold font-mono uppercase tracking-wide text-black bg-[#C8F135] shrink-0 ml-1">
        ...
        <p className="text-center text-xs text-slate-400 font-mono mb-20">
          Already on Foundation, Edge, or Floor? <span className="text-white font-bold">Signal Centre live feed is 100% included at £0 extra cost.</span>
        </p>

        {/* ── PDF EBOOKS & MANUALS DOWNLOADS SECTION ── */}
        <section id="downloads" className="pt-16 pb-16 border-t border-slate-800/80 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {PDF_BOOKS.map((book) => (
              <div key={book.id} className="bg-[#070A12] border border-slate-800 rounded-xl p-6 flex flex-col justify-between space-y-6 hover:border-slate-700 transition-colors">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded border border-slate-700 text-slate-300">
                      {book.tags[0]}
                    </span>
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-white font-sans">{book.title}</h3>
                    <p className="text-xs text-[#C8F135] font-mono font-semibold mt-0.5">{book.subtitle}</p>
                  </div>
                ...
                <div className="space-y-3 pt-4 border-t border-slate-800">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <a
                      ...
                      download
                      className="py-2.5 px-3 bg-slate-900 border border-slate-700 hover:border-slate-500 text-slate-200 text-center font-mono font-bold text-[11px] uppercase tracking-wider rounded flex items-center justify-center gap-1.5 transition-colors"
                    >
                    ...
                    <Link
                      href={book.storeUrl}
                      className="py-2.5 px-3 bg-[#C8F135] text-black font-mono font-extrabold text-[11px] uppercase tracking-wider rounded text-center flex items-center justify-center gap-1 hover:bg-[#b3d82a] transition-colors"
                    >
                      Full Book →
                    </Link>
                  </div>
          ...
          <div className="bg-[#070A12] border border-slate-800/80 rounded-xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
              {FREE_RESOURCES.map((item, idx) => (
                <a
                  ...
                  download
                  className="p-4 bg-[#090D18] border border-slate-800 rounded-lg hover:border-[#C8F135]/40 transition-all flex flex-col justify-between space-y-3 group"
                >
                  <div className="space-y-1">
                    <span className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">{item.format} • {item.size}</span>
                    <h4 className="text-xs font-bold text-white group-hover:text-[#C8F135] transition-colors font-sans">{item.title}</h4>
                  </div>

                  <div className="flex items-center gap-1 text-[11px] font-mono text-[#C8F135] font-bold">
        ...
        {/* Educational Notice */}
        <div className="mt-12 p-6 bg-[#070A12] border border-slate-800/80 rounded-xl max-w-4xl mx-auto text-center space-y-2">

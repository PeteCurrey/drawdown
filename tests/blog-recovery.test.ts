...
const OCTOBER_2026_SLUGS = [
  'q3-earnings-season-playbook-2026',
  'uk-autumn-budget-2026-trader-implications',
  'us-nonfarm-payrolls-october-2026-fed-implications',
  'bank-of-japan-normalisation-yen-carry-october-2026',
  'gold-breakout-2700-structure-2026',
  'prop-firm-rule-changes-october-2026',
  'algorithmic-execution-latency-slippage-retail-traders',
  'drawdown-october-2026-platform-update',
];

test('Blog System: getAllPosts returns at least 108 published posts in strict descending order', async () => {
  const posts = await getAllPosts();
  assert.ok(posts.length >= 108, `Expected at least 108 published posts after October batch, got ${posts.length}`);
  ...
  // Latest post should be from October 2026
  const latestYear = new Date(posts[0].publishedAt).getFullYear();
  const latestMonth = new Date(posts[0].publishedAt).getMonth() + 1;
  assert.equal(latestYear, 2026, `Latest post must be from 2026`);
  assert.equal(latestMonth, 10, `Latest post must be from October 2026`);
...
test('Blog System: all 8 October 2026 articles resolve with full body, author, and SEO', async () => {
  for (const slug of OCTOBER_2026_SLUGS) {
    const post = await getPostBySlug(slug);
    assert.ok(post, `October editorial post "${slug}" must exist`);
    assert.equal(post.slug, slug);
    assert.ok(post.title.length > 10, `Post "${slug}" title must be substantive: ${post.title}`);
    assert.ok(post.content.length > 1000, `Post "${slug}" must have comprehensive depth (len=${post.content.length})`);
    assert.equal(post.author, 'Pete Currey', `Post "${slug}" author must be Pete Currey`);
    ...
    // Verify dates are in October 2026
    const pubDate = new Date(post.publishedAt);
    assert.equal(pubDate.getFullYear(), 2026, `Post "${slug}" must be published in 2026`);
    const month = pubDate.getMonth() + 1;
    assert.equal(month, 10, `Post "${slug}" must be published in October 2026 (month=${month})`);
  }
});
...
test('Blog System: sitemap dynamically includes all 108+ published blog posts', async () => {

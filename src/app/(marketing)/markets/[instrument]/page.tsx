      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-end mb-24">
          <div>
            ...
            <h1 className="text-6xl md:text-8xl font-display font-bold uppercase mb-8 text-text-primary leading-[0.85]">
              {instrument.name}.
            </h1>
            <p className="text-2xl text-text-secondary leading-tight max-w-xl uppercase font-display">
               {instrument.description}
            </p>
          </div>

          {/* Live Data Panel Mockup */}
          <div className="bg-background-surface border border-border-slate p-10 relative overflow-hidden group">
             ...
             <div className="space-y-8 relative z-10">
                ...
                <div className="h-48 bg-background-primary/50 border border-border-slate/50 relative overflow-hidden">
                ...
                <div className="flex gap-4">
                   <Link 
                     href="/signup"
                     className="flex-1 py-4 bg-accent text-background-primary text-center text-[10px] font-bold uppercase tracking-widest hover:invert transition-all"
                   >
                      Trade {instrument.symbol}
                   </Link>
                   <button className="px-6 border border-border-slate hover:border-text-primary transition-colors">
        ...
        <section className="mb-32">
           <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-border-slate border border-border-slate">
              {[
                ...
                <div key={i} className="bg-background-surface p-10 space-y-4">
                   <div className="flex items-center gap-3 text-text-tertiary">
                      <fact.icon className="w-4 h-4" />
                      <span className="text-[10px] font-mono uppercase tracking-widest">{fact.label}</span>
                   </div>
                   <p className="text-xl font-display font-bold uppercase text-text-primary">{fact.value}</p>
                </div>
              ))}
           </div>
        </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 items-start mb-32">
          <div className="lg:col-span-2 space-y-24 prose prose-invert max-w-none">
            ...
            <section id="what-is" className="space-y-8">
              <h2 className="text-4xl font-display font-bold uppercase tracking-tight text-text-primary">
            ...
            <section id="what-drives" className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 {instrument.fundamentalDrivers.map((driver, i) => (
                   <div key={i} className="p-8 bg-background-surface border border-border-slate group hover:border-accent/30 transition-all">
                      <div className="text-accent font-mono text-xs mb-4">DRVR // 0{i+1}</div>
                      <h4 className="text-lg font-display font-bold uppercase text-text-primary mb-2">{driver}</h4>
            ...
            <section id="how-to-trade" className="space-y-8">
              ...
              <div className="space-y-6">
                 {instrument.tradingTips.map((tip, i) => (
                   <div key={i} className="flex gap-6 p-8 bg-background-surface border border-border-slate">
                      <div className="flex-shrink-0 w-12 h-12 bg-background-primary border border-border-slate flex items-center justify-center text-accent font-mono font-bold">
                         {i + 1}
                      </div>
            ...
            <section id="best-strategy" className="space-y-8">
               ...
               <div className="p-10 bg-accent/10 border border-accent/30 flex flex-col md:flex-row gap-8 items-center">
                  <div className="flex-1">
                     <h4 className="text-2xl font-display font-bold uppercase text-accent mb-2">Master {instrument.name} Today</h4>
                     <p className="text-sm text-text-secondary">
                        Our {instrument.relevantCoursePhase} covers specific institutional strategies for ${instrument.symbol} in depth.
                     </p>
                  </div>
                  <Link href="/learn-to-trade" className="px-8 py-4 bg-accent text-background-primary text-[10px] font-bold uppercase tracking-widest hover:invert transition-all whitespace-nowrap">
                     Access Curriculum
                  </Link>
               </div>
            </section>
          ...
          <aside className="sticky top-32 space-y-12">
            {/* Broker Card */}
            <div className="p-10 bg-background-surface border border-border-slate hover:border-accent/30 transition-premium group">
               <h4 className="text-[10px] font-mono uppercase tracking-widest text-text-tertiary mb-8">Recommended Broker</h4>
               <div className="space-y-6">
                  <div className="w-full aspect-video bg-background-primary border border-border-slate flex items-center justify-center mb-6">
                     <span className="text-2xl font-display font-bold uppercase tracking-tighter opacity-50">{instrument.bestBrokerId}</span>
                  </div>
                  <p className="text-sm text-text-secondary leading-relaxed">
                     Trade {instrument.name} with institutional spreads and lightning-fast execution on {instrument.bestBrokerId}.
                  </p>
                  <Link href={`/brokers/${instrument.bestBrokerId}`} className="block w-full py-4 border border-border-slate text-center text-[10px] font-bold uppercase tracking-widest group-hover:border-accent group-hover:text-accent transition-all">
                     View Full Review
                  </Link>
               </div>
            </div>
        ...
        {/* Localized FAQ */}
        <section className="mb-32">
           <h2 className="text-4xl font-display font-bold uppercase mb-16 text-text-primary">Frequently Asked Questions.</h2>
           <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border-slate border border-border-slate">
              {faqSchema.mainEntity.map((faq, i) => (
                <div key={i} className="bg-background-surface p-10 space-y-4 hover:bg-background-elevated transition-colors">
                   <h4 className="text-xl font-display font-bold uppercase text-text-primary leading-tight">{faq.name}</h4>
        ...
        {/* Global CTA */}
        <section className="p-20 bg-background-surface border border-border-slate relative overflow-hidden group">
           <div className="relative z-10 max-w-2xl">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                 <Link 
                   href="/signup"
                   className="px-12 py-6 bg-accent text-background-primary text-[12px] font-bold uppercase tracking-widest hover:invert transition-all text-center"
                 >
                    Join Drawdown Free
                 </Link>
                 <Link 
                   href="/tools/market-charts"
                   className="px-12 py-6 border border-border-slate text-text-primary text-[12px] font-bold uppercase tracking-widest hover:border-text-primary transition-all text-center"
                 >
                    View Global Charts
                 </Link>
              </div>
           ...
           <div className="absolute right-0 bottom-0 text-[400px] font-display font-black text-text-primary/5 select-none leading-none -mr-20 -mb-20 uppercase">

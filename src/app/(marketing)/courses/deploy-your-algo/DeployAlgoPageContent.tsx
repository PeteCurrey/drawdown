...
interface DeployAlgoPageContentProps {
  accessState: 'no-auth' | 'no-access' | 'has-access';
}
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-2">
            {accessState === 'has-access' ? (
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <Link
                  href="/dashboard/courses/deploy-your-algo"
                  className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-[#C8F135] text-black font-bold text-sm rounded-xl hover:bg-[#b8e020] active:scale-95 transition-all"
                >
                  Access Your Course →
                </Link>
                <div
                ...
                >
                  Included in your Floor membership ✓
                </div>
              </div>
            ) : (
              <>
                ...
                {accessState === 'no-auth' ? (
                  <Link
                    href="/login?redirect=/courses/deploy-your-algo"
                  ...
                  >
                    Floor Members — Sign in for free access
                  </Link>
                ) : (
                  <Link
                    href="/pricing"
                    className="inline-flex items-center justify-center gap-2 px-8 py-4 border border-white/20 text-white/60 font-bold text-sm rounded-xl hover:border-white/40 hover:text-white/80 transition-all"
                  >
                    On the Floor plan? Your course is included.
                  </Link>
                )}
              </>
            )}
          ...
          <div className="bg-white/3 border border-white/10 rounded-2xl p-8 space-y-6">
            ...
            {accessState === 'has-access' ? (
              <div className="space-y-4">
                <Link
                  href="/dashboard/courses/deploy-your-algo"
                  className="w-full py-4 bg-[#C8F135] text-black font-bold rounded-xl hover:bg-[#b8e020] active:scale-95 transition-all text-base inline-flex items-center justify-center font-display uppercase tracking-wider"
                >
                  Access Your Course →
                </Link>
                <div
                  className="w-full py-3 rounded-xl border text-center text-xs font-mono"
                  style={{
                    backgroundColor: "#111",
                    borderColor: "#22C55E",
                    color: "#22C55E",
              ...
              <>
                ...
                {accessState === 'no-auth' ? (
                  <p className="text-center text-[11px] text-white/30">
                    ...
                    <Link href="/login?redirect=/courses/deploy-your-algo" className="text-[#C8F135] hover:underline">
                  ...
                  <p className="text-center text-[11px] text-white/30">
                    On the Floor plan?{" "}
                    <Link href="/pricing" className="text-[#C8F135] hover:underline">

...
export default function UnitedStatesPricingPage() {
  ...
  const handleSubscribe = async (tierId: string) => {
    ...
    try {
      const plan = (US_PRICING as any)[tierId];
      const stripePriceId = plan.stripePriceId;

      const response = await fetch("/api/stripe/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ priceId: stripePriceId, tier: tierId, region: 'US' }),
      });
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-20">
          ...
          <p className="text-mkt-i2 max-w-2xl mx-auto mb-12">
            Institutional-grade education for US traders. USD-normalized pricing and SEC/CFTC compliant tools.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
          {tiers.map((tier) => {
            const plan = (US_PRICING as any)[tier.id];
            return (
              <div 
                key={tier.id}
                className={cn(
        ...
        <div className="mt-24 p-12 bg-[#F7F7F7] border border-mkt-bd max-w-4xl mx-auto flex flex-col md:flex-row gap-12 items-center">
           ...
           <div className="space-y-4 text-center md:text-left">
              <h4 className="text-xl font-sans font-bold uppercase tracking-widest text-mkt-ink">SEC & CFTC Compliance</h4>

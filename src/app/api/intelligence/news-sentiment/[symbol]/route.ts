...
const AV_MAP: Record<string, string> = {
  ...
  USDJPY:  "FOREX:JPY",
  GBPJPY:  "FOREX:GBP",
  XAUUSD:  "PHYSICAL_CURRENCY:XAU",
  XAGUSD:  "PHYSICAL_CURRENCY:XAG",
  WTI:     "COMMODITY:WTI",
  BRENT:   "COMMODITY:BRENT",
  BTCUSDT: "CRYPTO:BTC",
  BTCUSD:  "CRYPTO:BTC",
  ETHUSDT: "CRYPTO:ETH",
  ETHUSD:  "CRYPTO:ETH",
...
// Keyword filter for news articles
const KEYWORDS: Record<string, string[]> = {
  EURUSD:  ["EUR", "euro", "ECB", "eurozone", "European", "dollar"],
  GBPUSD:  ["GBP", "pound", "sterling", "BOE", "Bank of England", "cable", "UK"],
  USDJPY:  ["JPY", "yen", "BOJ", "Bank of Japan"],
  GBPJPY:  ["GBP", "JPY", "pound", "yen"],
  XAUUSD:  ["gold", "XAU", "precious metal", "bullion", "inflation", "Fed"],
  XAGUSD:  ["silver", "XAG", "precious metal", "metals"],
  WTI:     ["oil", "crude", "WTI", "energy", "OPEC", "petroleum"],
  BRENT:   ["oil", "crude", "Brent", "energy", "OPEC"],
  ...
  SPX:     ["S&P", "SP500", "stocks", "equities", "Fed", "Wall Street"],
...
export async function GET(
  ...
  const slug       = symbol.toUpperCase().replace("/", "");
  const finnhubKey = process.env.FINNHUB_API_KEY  ?? "";
  const avKey      = process.env.ALPHA_VANTAGE_KEY ?? "";
  const keywords   = KEYWORDS[slug] ?? [slug, "market", "trading"];
  const source_errors: string[] = [];
  const allArticles: Article[]  = [];

  // ─── Source 1: Real RSS Financial Feeds (ForexLive, Yahoo, BBC, CoinDesk, Investing.com) ───
  try {
    const rssArticles = await fetchNews();
    const relevantRss = rssArticles.filter(item => {
      const text = `${item.title} ${item.excerpt}`.toLowerCase();
      return keywords.some(kw => text.includes(kw.toLowerCase()));
    }).slice(0, 10);

    for (const n of relevantRss) {
      if (n.url && n.url !== "#") {
        allArticles.push({
          title: n.title,
          source: n.source,
    ...
    source_errors.push(`RSS: ${String(e)}`);
  }
        ...
        for (const item of data.feed ?? []) {
          if (item.url && item.url !== "#") {
            ...
            allArticles.push({
              ...
              published_at:    pub || new Date().toISOString(),
              url:             item.url,
  ...
  // ─── Deduplicate and Filter out non-functional URLs ────────────────────────
  const seen = new Set<string>();
  const deduped = allArticles
    .filter(a => {
      if (!a.url || a.url === "#" || a.url.trim() === "") return false;

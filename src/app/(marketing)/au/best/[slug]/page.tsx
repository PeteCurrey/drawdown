...
export default async function AustralianBestOfPage({ params }: Props) {
  const { slug } = await params;
  const page = BEST_OF_PAGES_AU.find((p) => p.slug === slug);
  ...
  return (
    <RegionalProvider region="au">
      <main className="min-h-screen bg-background-primary pt-32 pb-20 px-6">
        <TrackPageView path={`/au/best/${slug}`} />
        <div className="max-w-4xl mx-auto">
          ...
          <nav className="flex items-center space-x-2 text-[10px] font-mono uppercase tracking-widest text-text-tertiary mb-8">
            ...
            <Link href="/au/best" className="hover:text-accent transition-colors">Best</Link>
            <ChevronRight className="w-3 h-3" />
          ...
          <section className="bg-accent p-12 text-center space-y-6">
            <h2 className="text-4xl font-display font-bold text-background-primary uppercase">Trade properly in Australia.</h2>

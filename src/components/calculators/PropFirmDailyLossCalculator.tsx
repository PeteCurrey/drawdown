...
export function PropFirmDailyLossCalculator() {
      ...
      <div className="lg:col-span-7 flex flex-col justify-between gap-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
          <div className="p-8 border border-border-slate/50 bg-background-primary/30 flex flex-col justify-between hover:border-accent transition-colors">
            <span className="text-[9px] font-mono text-text-tertiary uppercase tracking-widest">Daily Loss Floor</span>
            <div className="mt-8">
              <p className="text-3xl font-sans font-black text-red-500">${dailyLossFloor.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              <p className="text-[9px] font-mono text-text-tertiary mt-2 uppercase tracking-widest">ACCOUNT HARD BREACH LEVEL</p>
            </div>
          </div>
          ...
          <div className="p-8 border border-border-slate/50 bg-background-primary/30 flex flex-col justify-between hover:border-accent transition-colors">
            ...
            <div className="mt-8">
              ...
              <p className="text-[9px] font-mono text-text-tertiary mt-2 uppercase tracking-widest">{percentToBreach.toFixed(2)}% TOLERANCE LEFT TODAY</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 border border-accent/20 bg-accent/5 flex items-start gap-4">
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-tight text-text-primary">Important Prop Firm Distinction</h4>
            <p className="text-[11px] text-text-secondary leading-relaxed mt-1">
              Most prop firms (e.g. FTMO, FundingPips) evaluate daily drawdown against <strong>Balance or Equity (whichever was higher at midnight CE(S)T)</strong>. Furthermore, floating losses count toward your daily loss limit. Always verify whether your firm resets at 00:00 CE(S)T, 00:00 UTC, or market close.
            </p>
          </div>
        </div>
      </div>
